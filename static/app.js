...
import externalTasksModule from './js/externalTasks.js';
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  // External Tasks tool button
  const toolExtTasksBtn = el('tool-ext-tasks-btn');
  if (toolExtTasksBtn) {
    toolExtTasksBtn.addEventListener('click', () => {
      if (externalTasksModule) {
        externalTasksModule.isPanelOpen() ? externalTasksModule.closePanel() : externalTasksModule.openPanel();
      }
    });
  }
