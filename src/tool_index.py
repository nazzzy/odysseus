...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_external_tasks": "Manage tasks synced from external sources (e.g. Todoist). Use 'list' to see tasks filtered by source or status. Use 'create' to add a task locally (pushed to remote on next sync). Use 'complete' to mark done. Use 'update' to change title, body, due_date, priority, or labels. Use 'sync' to trigger immediate pull+push. Do NOT also call api_call for the same action — sync_pending handles the push automatically.",
